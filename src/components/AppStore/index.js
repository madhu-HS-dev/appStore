...
class AppStore extends Component {
  state = {activeId: tabsList[0].tabId, searchApps: ''}
  ...
  render() {
    const {activeId, searchApps} = this.state

    const filteredList = appsList.filter(each => each.category === activeId)

    const filteredSearchApps = filteredList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(searchApps.toLowerCase()),
    )
      ...
      <div className="appStore-container">
        ...
        <ul className="tabItem-container">
          {tabsList.map(eacTabItem => (
            <TabItem
              tabsListDetails={eacTabItem}
              key={eacTabItem.tabId}
              onChangeTabs={this.onChangeTabs}
            />
          ))}
        </ul>
        <ul className="appItem-container">
          {filteredSearchApps.map(eachAppItem => (
            <AppItem appsListDetails={eachAppItem} key={eachAppItem.appId} />
          ))}
        </ul>
      </div>
    )
